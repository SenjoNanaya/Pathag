...
class MapTest extends StatefulWidget {
  ...
  const MapTest({super.key, this.currentIndex = 1});
...
class _MapTest extends State<MapTest> {
  ...
  Future<List<LatLng>> _calculateRoute(LatLng origin, LatLng destination) async {
    ...
    // 1. Simulate network "Thinking" time (800ms)
    await Future.delayed(const Duration(milliseconds: 800));

    // 2. Generate a mid-point with a "Wiggle" to simulate campus paths 
    // rather than a perfectly straight line.
    LatLng midPoint = LatLng(
      (origin.latitude + destination.latitude) / 2 + 0.0003, 
      (origin.longitude + destination.longitude) / 2 + 0.0003
    );
