...
def _make_loaders(
    ...
    n_classes = len(OBSTACLE_TYPE_CLASS_ORDER)
...
def run_training(
    *,
    ...
    if use_class_weighted_loss:
        ...
        if inv_sum > 0:
            class_weight_tensor = (inv / inv_sum * len(OBSTACLE_TYPE_CLASS_ORDER)).to(device)
    ...
    for epoch in range(epochs):
        ...
        correct = 0
        total = 0
        per_class_correct = torch.zeros(len(OBSTACLE_TYPE_CLASS_ORDER), dtype=torch.long)
        per_class_total = torch.zeros(len(OBSTACLE_TYPE_CLASS_ORDER), dtype=torch.long)
        with torch.no_grad():
        ...
        per_class_parts = []
        for idx, cls_name in enumerate(OBSTACLE_TYPE_CLASS_ORDER):
        ...
        if acc > best_val_acc:
            best_val_acc = acc
            best_epoch = epoch + 1
            # Move weights to CPU for safe checkpoint persistence.
